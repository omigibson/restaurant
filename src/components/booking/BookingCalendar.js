...
// Selectors
import { getDaysThatAreFull } from '../../selectors/calendarSelectors';
...
class BookingCalendar extends React.Component {
  ...
  componentWillMount = () => {
      // this.props.setAppState({ progressBar: 33 });
       // this.setState({ allBookings: this.props.bookings }, () => {
 ...
 componentDidMount() {
        ...
        if (!item.classList.contains("booked-day")) {
          if (!this.controlIfDateIsPassed(todayWithHourRestriction)) {
            item.addEventListener("click", () => {
                this.props.updateViewstate('dateSelected', todaysFullDate);
  ...
  render = () => {
    ...
    if ( this.state.stepCompleted ) {
      return <Redirect to={'/booking/choosetime'} />
    }
          ...
          <React.Fragment>
            ...
            { styles =>
              <div className="booking-step container flex vcenter column" style={styles}>
                ...
                <Calendar
                  disableHistory={true}
                  bookings={this.props.bookedDates}
                  clickable={true}
                />
...
export default connect(BookingCalendar, { updateViewstate }, (store) => ({
  ...
  daysThatAreFull: getDaysThatAreFull(store)
}))
